 ...
 */ 
$header = '<h2 class="font-bold text-2xl text-slate-800 leading-tight">Vima Route Authorization Guard</h2>';
?>
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-3">
                ...
                <div class="space-y-2">
                    <div>
                        ...
                        <code class="bg-slate-100 px-1.5 py-0.5 rounded text-indigo-600 font-mono text-xs">dashboard/admin-only</code>
                    </div>
                    <div>
                        <span class="block text-xxs font-bold text-slate-400 uppercase tracking-wider">Applied Filter</span>
                        <code class="bg-slate-100 px-1.5 py-0.5 rounded text-rose-600 font-mono text-xs">vima_rbac:admin</code>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 space-y-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-0.5">
                            You are seeing this page because the active user context contains the <strong class="font-semibold text-slate-700">admin</strong> role required by the route filter.
                        </p>
                    </div>
                </div>
                ...
                <div class="border-t border-slate-100 pt-6">
                    ...
                    <div class="space-y-4 text-xs text-slate-600 leading-relaxed">
                        <p>
                            In CodeIgniter 4, we protect paths directly inside <code class="bg-slate-50 text-slate-700 px-1 py-0.5 rounded font-mono">app/Config/Routes.php</code> using the Vima RBAC route filter:
                        </p>
                        ...
                        <div class="bg-slate-900 rounded-xl p-4 text-slate-300 font-mono text-xs overflow-x-auto shadow-inner border border-slate-800">
                            ...
                            $routes->get(<span class="text-emerald-400">'admin-only'</span>, <span class="text-emerald-400">'Dashboard::adminOnly'</span>, [<br>
                            &nbsp;&nbsp;&nbsp;&nbsp;<span class="text-orange-400">'filter'</span> => <span class="text-emerald-400">'vima_rbac:admin'</span><br>
                            ]);
                        </div>

                        <p>
                            If you attempt to visit this route while simulating a user context that does not have the <code class="bg-slate-100 text-slate-800 px-1 py-0.5 rounded font-mono">admin</code> role (e.g., Manager or Viewer), the request is intercepted by <strong class="font-semibold text-slate-800">VimaRbacFilter</strong> before reaching the controller.
                        </p>
                    </div>
                </div>
                    ...
                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4">
                        <div class="flex flex-col md:flex-row items-center justify-between gap-4 font-mono text-xs text-slate-600">
                            <div class="flex-1 w-full text-center p-3 bg-white border border-slate-200 rounded-lg shadow-xxs">
                                ...
                                <div class="text-xxs text-slate-400 mt-1">/dashboard/admin-only</div>
                            </div>
                            ...
                            <div class="flex-1 w-full text-center p-3 bg-indigo-50 border border-indigo-200 rounded-lg shadow-xxs">
                                <span class="font-semibold text-indigo-700">VimaRbacFilter</span>
                                <div class="text-xxs text-indigo-500 mt-1">Check role `admin`</div>
                            </div>
                            <div class="text-slate-400 hidden md:block">➔</div>
                            <div class="flex-1 w-full text-center p-3 bg-emerald-50 border border-emerald-200 rounded-lg shadow-xxs">
                                ...
                                <div class="text-xxs text-emerald-500 mt-1">Dashboard::adminOnly()</div>
                            </div>
                        </div>
                    </div>
                </div>
